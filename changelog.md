...
- Extended the index schema with `definition` and `parents` columns (appended after the db columns, so a file written before them still parses). `load_index` reads them defensively and records a `has_details` flag; the endpoint reports `details_available: false` — with a note — for an index that predates the columns, rather than pretending the term has no definition. `scripts/fetch_databases.py` now captures OBO `def:`/`is_a:` (parents resolved to labels in a second pass), MeSH scope notes, and Orphanet definitions.
- Prediction behaviour, confidence rules and SSSOM output are unchanged — the schema change is additive (proved by a snapshot test over `predict_matches`).
- **Committed indexes are not regenerated in this change** (regenerating needs the multi-MB `raw/` dumps, and full definitions would grow MONDO/NCIt substantially): the compare pane works today off label + synonyms + cross-reference provenance and reports `details_available: false` for definitions until the indexes are rebuilt. If the size delta proves large, split details into a sidecar `<db>.details.tsv` — see `data/2-databases/README.md`.
